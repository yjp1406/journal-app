const queries = {
    ...
    publishJournal: "UPDATE journals SET published_at = CONVERT_TZ(NOW(), 'UTC', 'Asia/Kolkata') WHERE id = ?",
    teacherFeed: "SELECT * FROM journals WHERE created_by = ? ORDER BY published_at DESC",
    studentFeed: "SELECT * FROM journals WHERE FIND_IN_SET(?, tagged_students) > 0 AND published_at <= CONVERT_TZ(NOW(), 'UTC', 'Asia/Kolkata') ORDER BY published_at DESC",
